...
for i in range(parameter_len):
    ...
    for j in range(sim_time):
        ...
        data = gen_data(n, p, family=family, k=k, rho=0.50, sigma=2)
        test_data = gen_data(n, p, family=family, k=k, rho=0.50, beta=data.beta, sigma=2)
        
        model_pdas = PdasLogistic(path_type="seq", sequence=range(1, min(int(n/np.log(n)), p)), ic_type="ebic")         
        time_begin = time()
        ...
        # print(model_pdas.predict(test_data.x))
        y_ = model_pdas.predict(test_data.x)["Y"]
        pre_loss_pdas[j] = acc(test_data.y, y_)
        
        
        model_gpdas = PdasLogistic(path_type="pgs", s_min=1, s_max=int(min(int(n/np.log(n)), p)), ic_type="ebic")
        time_begin = time()
        ...
        y_ = model_gpdas.predict(test_data.x)["Y"]
        pre_loss_gpdas[j] = acc(test_data.y, y_)
        
        model_glmnet = LogitNet(n_lambda=100, alpha=1, n_splits=5)
        time_begin = time()
        data.y = data.y.reshape(-1)
#        print(data.y.shape)
#        print(data.x.shape)
        model_glmnet.fit(data.x, data.y)
        
        t_glmnet[j] = time() - time_begin
        model_glmnet_beta, _ = model_glmnet.get_beta()
        
        # print(model_glmnet_beta.shape)
...
#        loss_glmnet[j] = model_glmnet.train_loss
        ...
        y_ = model_glmnet.predict(test_data.x)
        pre_loss_glmnet[j] = acc(test_data.y, y_)
...
result.to_csv('simulate_result_' + family + '_' + str(parameter_len) + '_'+str(sim_time) + '.csv')