...
def CV(data, kfold, lamb):
    ...
    for train_index, test_index in kf.split(data.x):
        ...
        fit = glmnet(x=x_train.copy(), y=y_train.copy(), family="cox", lambdau=np.array([lamb]))
        model_beta = fit["beta"]      
...
#     f.write(",t_pdas_list,TP_pdas_list,FP_pdas_list,mcc_pdas_list,pre_loss_pdas_list,t_gpdas_list,TP_gpdas_list,FP_gpdas_list,mcc_gpdas_list,pre_loss_gpdas_list,t_glmnet_list,TP_glmnet_list,FP_glmnet_list,mcc_glmnet_list,pre_loss_glmnet_list")
...
for i in range(parameter_len):
    ...
    for j in range(sim_time):
        ...
        data = gen_data(n, p, family=family, k=k, rho=0.50, sigma = 1, c=50, scal=10)
        test_data = gen_data(n, p, family=family, k=k, rho=0.50, sigma = 1, beta=data.beta, c=50, scal=10)
        

        model_pdas = PdasCox(path_type="seq", sequence=range(1, min(int(100), p)), ic_type="ebic")         
        time_begin = time()
        ...
        model_gpdas = PdasCox(path_type="pgs", s_min=1, s_max=int(min(int(100), p)), ic_type="ebic")
        time_begin = time()
        ...
        fit = glmnet(x=data.x, y=data.y, family="cox", lambdau= np.array([lamb_best]))
        model_beta = fit["beta"]
        t_glmnet[j] = time() - time_begin

    #     model_glmnet_beta = model_beta
