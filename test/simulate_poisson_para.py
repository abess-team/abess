...
def sim(n,p,k,j,family,sigma,rho):
    ...
    # model_pdas = PdasPoisson(path_type="seq", sequence=range(1, min(p, int(n/np.log(n)))), is_cv=True, K=5)        
    # time_begin = time()
    ...
    # model_gpdas = PdasPoisson(path_type="pgs", s_min=1, s_max=int(min(p, int(n/np.log(n)))), is_cv=True, K=5)
...
if __name__ == "__main__":
    ...
    logging.basicConfig(level=logging.DEBUG,
                ...
                filename='simulate_' + family + '_' + str(parameter_len) + '_'+str(sim_time) + '_cv.log',
                filemode='w')

    file_name= 'simulate_' + family + '_' + str(parameter_len) + '_'+str(sim_time) + '_cv.csv'
