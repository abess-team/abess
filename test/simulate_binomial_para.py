...
def sim(n,p,k,j,family,sigma,rho):
    ...
    model_pdas = PdasLogistic(path_type="seq", sequence=range(1, min(p, int(n/np.log(n)))), ic_type="aic")
    # model_pdas = PdasLogistic(path_type="seq", sequence=range(1, min(p, int(n/np.log(n)))), is_cv=True, K=5)         
    time_begin = time()
    ...
    model_gpdas = PdasLogistic(path_type="pgs", s_min=1, s_max=int(min(p, int(n/np.log(n)))), ic_type="aic")
    # model_gpdas = PdasLogistic(path_type="pgs", s_min=1, s_max=int(min(p, int(n/np.log(n)))), is_cv=True, K=5)
    time_begin = time()
