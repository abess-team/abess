...
for i in range(parameter_len):
    ...
    for j in range(sim_time):
        ...
        data = gen_data(n, p, family=family, k=k, rho=0.5, sigma=9)
        test_data = gen_data(n, p, family=family, k=k, rho=0.5, sigma=9, beta=data.beta)
        
        model_pdas = PdasLm(path_type="seq", sequence=range(1, min(p, int(n/2))), ic_type="ebic")         
        time_begin = time()
        ...
        model_gpdas = PdasLm(path_type="pgs", s_min=1, s_max=int(min(p, int(n/2))), ic_type="ebic")
        time_begin = time()
        ...
        model_glmnet = ElasticNet(n_lambda=100, alpha=1, n_splits=5)
        time_begin = time()
