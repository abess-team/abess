...
the estimator obtained by abess package shows the best prediction performance. Second, the estimator obtained by abess package 
can reasonably control the false-positive rate 
at a low level like SCAD and MCP while prevent the poor true signal detection performance as SCAD and MCP in the linear model when the correlation between predictors is high. 
Furthermore, our abess package is highly efficient compared with 
other packages. To conduct the same task, the abess package is never eclipsed by the glmnet pacakge which is esteemed as the benchmark of efficiency.
