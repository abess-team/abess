...
print(best_size)
print(coef(abess_fit, support.size = best_size)[1:10])
```
...
The pre-processed dataset for statistical modeling has 200 observations and 5253 predictors, and the pre-processed details are presented in [https://github.com/Mamba413/abess/tree/master/R-package/data-raw](https://github.com/Mamba413/abess/tree/master/R-package/data-raw).   
...
```{r}
best_coef <- coef(abess_fit, abess_fit[["best.size"]])
best_vars <- best_coef@Dimnames[[1]][best_coef@i[-1]]
best_vars
```
...
library(reshape2)
library(ggplot2)
pdat <- cbind.data.frame(crime[, c("y",
                                   "pctKidsBornNevrMarr:pctHousOwnerOccup", 
                                   "pct65up:pctMaleNevMar")])
pdat <- melt(pdat, id.vars = "y")
p <- ggplot(pdat) + geom_point(aes(value, y), size = 0.3) + 
  geom_smooth(aes(value, y), method = "lm", se = FALSE) +
  # geom_smooth(aes(value, y), color = "red") + 
