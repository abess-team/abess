...
## Brief introduction       
The R package `abess` implement a polynomial algorithm [\color{blue}{paper}](https://doi.org/10.1073/pnas.2014241117) for solving the best-subset selection problem:
...
By setting the arguments `support.size = s`, `abess()` function conducts **Algorithm 1** [\color{blue}{paper}](https://doi.org/10.1073/pnas.2014241117) for the best-subset selection with a sparsity level `s`. In our example, we set the options: `support.size = 3`, and we run **Algorithm 1** with the following command: 
```{r}
...
Supposing we are unknown about the true sparsity level in real world data, and thus, we need to determine the most proper one. The **Algorithm 3** [\color{blue}{paper}](https://doi.org/10.1073/pnas.2014241117) is designed for this scenario. `abess` is capable of performing this algorithm: 
```{r}
