...
This vignette introduces what is adaptive best subset selection robust principal component analysis and then we will show how it works using *abess* package on an artifitial example.
...
X=S+L,
$$
where $S$ is the sparse "outlier" matrix and $L$ is the "information" matrix with a low rank. Generally, we also suppose $S$ is not low-rank and $L$ is not sparse, in order to get only solution.
In Largrange format,
$$
...
3. For $i=1,2, \ldots, M$
A. $X_{o l d}=\left\{\begin{array}{ll}X_{\text {new }}, & \text { for outliers } \\ X, & \text { for others }\end{array}\right.$
B. Form KPCA on $X_{o l d}$ with $K=r$, and denote $v$ as the eigenvectors;
C. $X_{\text {new }}=X_{\text {old }} \cdot v \cdot v^{T}$;
D. If $\left\|X_{n e w}-X_{o l d}\right\|<\varepsilon$, break:
End for;
4. Return $X_{\text {new }}$ as $L$;
where $M$ is the maximum iteration times and  is the convergence coefficient.

The final $X_{\text {new }}$ is supposed to be $L$ under given outlier positions.

## RPCA Application

ecently, RPCA is more widely used, for example,
