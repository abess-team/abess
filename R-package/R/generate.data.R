...
#' \code{"ordinal"} for ordinal response.
...
#' 
#' For \code{family = "gamma"}, the data is modeled to have
#' a gamma distribution:
#' \deqn{Y = Gamma(X \beta + \epsilon + 10, shape),}
#' where \eqn{shape} is shape parameter in a gamma distribution.
#' The underlying regression coefficient \eqn{\beta} has
#' uniform distribution [2m, 100m] and \eqn{m = \sqrt{2log(p)/n}.}
#' 
#' For \code{family = "ordinal"}, the data is modeled to have
