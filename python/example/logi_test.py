...
for m in range(M):
    ...
    if "lasso" in method:
        ...
        t_start = time()
        model = LogisticRegressionCV(penalty = "l1", solver = "liblinear", cv = 5, n_jobs = -1)
        fit = model.fit(train.x, train.y)
        t_end = time()

        met[ind, m, 0:5] = metrics(fit.coef_.flatten(), fit.predict(test.x), test)
    ...
    if "abess" in method:
        ...
        model = abessLogistic(is_cv = True, support_size = range(100), thread = 0, 
