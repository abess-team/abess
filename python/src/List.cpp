...
void List::add(string name, Matrix<long double, Dynamic, 1> &value)
{
    ...
    {
        if (vector_Vector_long_name[i] == name)
        {
            vector_Vector_long[i] = value;
            return;
        }
    }
    vector_Vector_long.push_back(value);
    vector_Vector_long_name.push_back(name);
}
