...
# With argument ``snr`` in ``make_glm_data()``, we can add white noise into generated data.
import numpy as np
...
for i in range(100):
    np.random.seed(i)
    data = make_glm_data(n = 200, p = 30, k = 5, family = 'gaussian', coef_ = coef, snr = 0.5, sigma=2, rho=0.5)
    train_x, test_x, train_y, test_y = train_test_split(data.x, data.y, test_size=0.5, random_state=i)
