...
# These features may also improve the statistical accuracy and algorithmic stability.
...
# and set ``screening_size = 100`` to maintain the 100 features with the largest marginal utilities.
...
# After that, the active set on the stable subset would be treated as that on the full set.
...
# This dataset comprise 200 observations, and each observation has 5000 features. 
...
print("support set (without important searching):\n", np.nonzero(model2.coef_)[0])
...
# Therefore, we recommend use important searching for large :math:`p` situation.
...
# In this situation, a lower ``important_search`` can save lots of time and space.
